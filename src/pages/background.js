document.addEventListener('DOMContentLoaded', function () {
	...
	function openMainView(sensePathMode, showBrowser, showVideo, showText) {
		...
		var captureMode = sensePathMode === 'capture' || !sensePathMode;
	    ...
	    if (!captureMode) {
		    if (showBrowser) { // Resize current window
			...
			if (showVideo) {
				chrome.windows.create({
			    	url: chrome.extension.getURL("src/pages/video.html"), type: "popup", left: showBrowser ? screen.width / 2 : 0, top: 0,
			    	width: showBrowser || showText ? screen.width / 2 : screen.width,
			    	height: (showBrowser && showText ? halfHeight : fullHeight) - titleBarHeight
			    });
			}

			if (showText) {
				chrome.windows.create({
			    	url: chrome.extension.getURL("src/pages/text.html"), type: "popup",
			    	left: showBrowser || showVideo ? screen.width / 2 : 0,
			    	top: Math.round(showBrowser && showVideo ? smTop / 2 - 2 : 0),
			    	width: showBrowser || showVideo ? screen.width / 2 : screen.width,
			    	height: (showBrowser && showVideo ? halfHeight : fullHeight) - titleBarHeight,
			    });
			}
		}
